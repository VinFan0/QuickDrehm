...
void QMC5883LCompass::read(){
	...
	if (!err) {
		Wire2.requestFrom(_ADDR, (byte)6);
		_vRaw[0] = (int)(int16_t)(Wire2.read() | Wire2.read() << 8);
		_vRaw[1] = (int)(int16_t)(Wire2.read() | Wire2.read() << 8);
		_vRaw[2] = (int)(int16_t)(Wire2.read() | Wire2.read() << 8);
